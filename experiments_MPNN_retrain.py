### Module implementing MPNN training from Volkov et al. 2022###
...
if __name__ == "__main__":
    ...
    #The hardcoded parameters are specific of the work of Volkov et al. 2022
    ...
    MODEL_NAME = "MPNN" #this script uses the MPNN defined by Volkov et al. 2022 for comparison purposes
    ...
    HIDDEN_CHANNELS = 2054
    EPOCHS = args["trainer"]["EPOCHS"]
    NODE_FEATURES = args["trainer"]["NODE_FEATURES"] #if False, use dummy features (1)
    BATCH_SIZE = 256 #args["trainer"]["BATCH_SIZE"]
    LEARNING_RATE = 0.01 #set as defalut, not specified in the paper
    LR_FACTOR = 0.9
    LR_INTERVAL = 20
    WEIGHT_DECAY = 0.001 #float(args["trainer"]["WEIGHT_DECAY"])
    ...
    # ### Define and train the MPNN as in Volkov et al. 2022

    class MPNN(MessagePassing):
        def __init__(self, hidden_size):
            super(MPNN, self).__init__(aggr='add')
            self.lin = nn.Linear(dataset[0].x.shape[1], hidden_size)
    ...
    class MPNN_Net(nn.Module):
        def __init__(self, hidden_size):
            super(MPNN_Net, self).__init__()
            self.mpnn = MPNN(hidden_size)
    ...
    model = MPNN_Net(HIDDEN_CHANNELS).to(device)

    lr = LEARNING_RATE

    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=WEIGHT_DECAY)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=LR_INTERVAL, gamma=LR_FACTOR)
